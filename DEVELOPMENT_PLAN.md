...
- 后台自动处理使用独立且默认关闭的用户开关；启用后以前台服务监听 MediaStore，以无网络约束的 WorkManager 每 15 分钟兜底发现并写入持久队列，再由带联网约束的任务消费。系统强制停止、厂商省电和前台服务政策仍可能造成延迟，不得宣称永不被杀。
...
使用 `ContentResolver.openInputStream()` 或文件描述符读取。编码、尺寸限制和网络请求所需的临时缓冲只能存在于内存中。默认不持久化原图；仅当用户开启“应用内保留原图副本”时，在成功入账后复制到应用私有目录。读取完成后立即关闭流并释放缓冲。
...
- 使用 HTTPS 的 OpenAI 兼容接口；显式支持 Responses API 与 Chat Completions，生产环境禁止明文 HTTP。
...
没有页面交易时间时，即时付款成功页读取截图左上角 Android 状态栏 `HH:mm`，与 MediaStore 截图时间的本地日期和时区组合，并标记 `SCREENSHOT_ESTIMATED`；状态栏时间不得伪装成 `PAGE_EXACT`。仅微信/支付宝原生即时付款成功页可在组合时间与截图元数据相差不超过新鲜度窗口、字段完整、置信度达标且无负特征时自动入账；状态栏不可读或冲突时仍需人工确认。所有自动入账、人工修改、失败重试、账单匹配和删除请求都写入审计事件。

## 5. 设置、结果提示与照片删除

- 设置页使用普通表单选择接口协议并输入 `Base URL`、`API Key`、`Model`、超时和云端识别开关，并提供连接测试；不显示或允许编辑原始请求 JSON。
...
- 首页提供最近七天账本与新增、修改、删除入口，并按日期展示 CNY 净支出、独立收入、盈余或超支；非 CNY 或字段不完整流水只展示、不混入预算。
- 月预算按自然日精确均摊，不能整除的分单位从月初依次补足；今日实时预览，过去日期次日方可结算。
- 正盈余可进入跨月预算池或独立存款台账；预算池由用户手动拨入今天或尚未结算的过去日期，不自动滚入次日。
- 预算池与存款使用只追加资金流水。迟到交易或预算修改使已结算日期进入待重新结算状态，重结算通过冲正保留历史版本；预算池调整欠额清零前禁止继续拨款。
- 识别页支持支付宝、微信支付官方 CSV/XLSX/ZIP 账单的本地格式验证、汇总预览、用户确认导入和交易指纹去重；失败、待支付、不计收支及异常行不入账。
- v0.4 支持手工新增、修改、删除账目和 CSV/XLSX 导出；存款取出、导入批次撤销和复杂冲突合并仍不包含。
...
- 净支出等于支出减退款；普通收入不改变每日预算结算，但会抵扣月度总预算占用，并另行展示现金流；
...
- 聊天截图、账单列表、历史详情、多金额、一般缺失时间和旧交易不得自动入账；微信即时付款成功页按明确的估算时间专用规则测试；
- 高置信度唯一支付页面只生成一条账目；
